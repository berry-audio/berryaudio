...
class StorageExtension(Actor):
    ...
    def on_dir(self, path: str):
        return list_paths(path,  extensions=[".mp3", ".m4a", ".flac", ".wav", ".ogg", ".dsf", ".m3u", ".m3u8"])
        ...
        if not self.is_internal(dev_node):
            ...
            try:
                ...
                if mount_point:
                    ...
                    self._core.send(event="storage_updated", storage=self.on_info(dev_node))
                    logger.debug(f"Mounted at {mount_point} - Total: {usage.total}, "
    ...
    def unmount_device(self, dev_node):
        if not self.is_internal(dev_node):
            ...
            try:
                ...
                self._core.send(event="storage_updated", storage=response)
