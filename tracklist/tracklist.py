...
class TracklistExtension(Actor):
    ...
    async def on_add(self, uris: list = []) -> bool:
        ...
        if uris:
            ...
            self._core.send(event='tracklist_changed',  tl_tracks=self._tl_tracks)       
            return _tracks
        else:
    ...
    async def on_set_repeat(self, value: bool) -> bool:
        ...
        self._repeat = value
        await self.on_next_track()
        self._core.send(event='options_changed') 
        return True
    ...
    async def on_next_track(self, from_ui: bool = False) -> TlTrack:
        ...
        """
        self._current_tltrack = await self._core.request("playback.get_current_tl_track")
