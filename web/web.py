...
class WebExtension(Actor):
    ...
    def _add_static_route(self, prefix: str, directory: pathlib.Path, mime_map: dict = None):
            ...
            if USE_GBULB:
                async def handler(request):
                    ...
                    content_type = mime_map.get(ext, "application/octet-stream") if mime_map else "application/octet-stream"
                    return await self.stream_file(request, file_path, content_type=content_type)
    ...
    async def run_server(self):
        ...
        async def index_handler(request):
            if USE_GBULB:
                return await self.stream_file(request, www_dir / "index.html", content_type="text/html")
            else:
                return web.FileResponse(www_dir / "index.html", headers={"Content-Type": "text/html"})
        ...
        for client in list(self._clients):
            try:
                if not client.closed:
                    await client.send_str(json.dumps(json.loads(handle_json_ws(message))))
    ...
    async def handle_jsonrpc(self, ws, data, send=True):
        try:
            ...
            if params is None:
                result = await self.core.request(method)
            elif isinstance(params, dict):
                result = await self.core.request(method, **params)
            else:
                raise ValueError("Params must be a JSON object (dict) or null")

            response = json.loads(handle_json({"jsonrpc": "2.0", "result": result, "id": req_id}))

        except Exception as e:
            response = json.loads(handle_json({
                "jsonrpc": "2.0",
                "error": {"code": -32603, "message": str(e)},
                "id": request_obj.get("id") if "request_obj" in locals() else None
            }))
