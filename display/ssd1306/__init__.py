...
from display.utils import format_time
...
class DisplaySSD1306:
    def __init__(self, contrast=255, core=None):
        self.core = core
        self.width = 128
        self.height = 64
        ...
        self._widget_title = WidgetTextScrollable(font_size=8, font_path=FONT_STYLE_3)
        self._widget_artist = WidgetTextScrollable(font_size=8, font_path=FONT_STYLE_3)
        self._widget_play_pause = WidgetPlayPause()
        self.list_scrollable = WidgetListScrollable(display_width=self.width, display_height=self.height, show_counter=False, font_size=8, font_path=FONT_STYLE_3)
        self.list_scrollable_source = WidgetListScrollable(
            display_width=self.width, display_height=self.height, show_counter=False, font_size=8, font_path=FONT_STYLE_3
        )
            ...
            with regulator:
                with canvas(self._device) as draw:
                    if self._page == "STANDBY":
                        if self._blink_visible:
                            ...
                            self._widget_title.draw(
                                draw, width=self.width, y=24, text=_time_now, center=True,
                            )

                    if self._page == "POWER_STATE_CHANGING":
                        self._widget_title.draw(
                            draw,
                            width=self.width,
                            y=24,
                            text=self._power_state,
                            auto_scroll=False,
                            center=True,
                        )
                    ...
                    if self._page == "VOLUME":
                        self._widget_title.draw(
                            draw, width=self.width, y=24, text=f"VOLUME {self._volume}", center=True,
                        )
                    ...
                    if self._page == "NOW_PLAYING":
                        ...
                        draw.bitmap((104, 0), Image.open(ICON_SPEAKER), fill="white")

                        # draw.bitmap((210, 0), blue_icon, fill="white")
                        # draw.bitmap((200, 0), airplay_icon, fill="white")
                        # draw.bitmap((190, 0), spotify_icon, fill="white")

                        draw.bitmap((91, 0), Image.open(ICON_REPEAT), fill="white")
                        draw.bitmap((77, 0), Image.open(ICON_SHUFFLE), fill="white")
