        ...
        while self.running:
            with regulator:
                with canvas(self._device) as draw:
                    if self._page == "STANDBY":
                        if self._blink_visible:
                            ...
                            self._widget_title.draw(
                                draw, width=self.width, x=0, y=12, text=_time_now, center=True
                            )
                    ...
                    if self._page == "VOLUME":
                        self._widget_title.draw(
                            draw, width=self.width, x=0, y=12, text=f"VOL {self._volume}" , center=True
                        )
                    ...
                    if self._page == "NOW_PLAYING" and not self._standby:
                        ...
                        if self._track is not None and self._track.name:
                            self._widget_title.draw(
                                draw, width=112, x=13, y=12, text=self._track.name
                            )
                        else:
