...
class DisplaySSD1322:
    ...
    def _display_loop(self):
        ...
        widget_artist = WidgetTextScrollable(self.device, screen_width=self.width, font_size=21, font_path=FONT_STYLE_1)
        # widget_artist = WidgetTextScrollable(self.device, screen_width=self.width, font_size=21, font_path=FONT_STYLE_2)
        ...
        while self.running:
            with regulator:
                with canvas(self.device) as draw:
                    widget_artist.draw(draw, width=256, x=-2, y=-3, text="Standby")
                    if self._track is not None:
                        widget_title.draw(draw, width=200, x=-3, y=12, text=self._track.name)

                        if len(self._track.artists):
                            widget_artist.draw(draw, width=256, x=-2, y=-3, text=next(iter(self._track.artists)).name)
                    ...
                    widget_vumeter.draw(draw, width=256, height=16, x=0, y=38,  bar_pattern='checkerboard')
